...
# headers are: answer, clue, year pubid, total = 6331681
nyt = pd.read_csv("C:/Users/31463/PycharmProjects/clues.tsv", sep='\t')

# restrict to year range
nyt = nyt[(nyt['year'] >= 2010) & (nyt['year'] <= 2018)]

# picks n rows at random
nyt = nyt.ix[np.random.choice(nyt.index, 20)]
nyt = nyt[['clue', 'answer']]

word_list = list(zip(nyt['answer'].values.tolist(), nyt['clue'].values.tolist()))
